...
1. Look at if a package ([vault](https://github.com/guacsec/guac-data/blob/main/top-dh-sboms/vault.json)) to see if it has an SBOM associated with and where it can be found:
  ```bash
...
2. Determine if an artifact has an SLSA attestation associated with it
  ...
  **Note**: Currently, the package "purl" is guessed based on heuristics. This
  ...
  attestation via the Node ID in the [graphQL playground]({{ site.baseurl }}{%link guac-graphql.md %})
...
3. Run the query on the Prometheus package we were working on within the workflow demo:
  ```bash
